---
layout: article
title: 适用于 MS Word 引文系统的华科特调文献引用格式实现与使用
key: ms-word-bibliography-styling
tags: Office Word
---

HUST special "GB/T 7714-2015"-like bibliography styling specification for
MS Word.
...
| ![](/assets/images/2023-4-4-msword-bib-sty/showcase.png) |
|:-----------:|
| Bibliography output |

This specification works with the MS Word citation & bibliography system
(引用-引文与书目-插入引文, not to be confused with ~~cross-referencing,
引用-题注-交叉引用~~), the not-so-convenient-however-canonical way where citations
are jammed into a shared citation source pool without any form of categorization
nor sorting options.

The specification provides support for the following citation source types

* 专利 (Patent)
* 期刊文章 (ArticleInAPeriodical)

    > Note that you cite books with source type 期刊文章/ArticleInAPeriodical
    > as well, since they are very much alike only the former omits several
    > fields, so I took the convenience to re-use the same code path.
...
Create an XSL file containing the code given in the appendix under the folder
`%APPDATA%\Microsoft\Bibliography\Style\`, then restart Word. A new citation &
bibliography style named "IEEE - modified" should show up.
...
Appendix: Bibliography styling code
-----------------------------------
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Start">
      ...
      <xsl:when test="b:StyleNameLocalized">
        <xsl:choose>
          <xsl:when test="b:StyleNameLocalized/b:Lcid='1033'"><!-- en-US -->
            <xsl:text>IEEE - modified</xsl:text>
          </xsl:when>
          ...
          <xsl:when test="b:StyleNameLocalized/b:Lcid='2052'"><!-- zh-CN -->
            <xsl:text>IEEE - modified</xsl:text>
          </xsl:when>
	...
	<xsl:template name="templ_str_InCap" >
		...
		<xsl:choose>
			<xsl:when test="$_LCID='1033'"><xsl:text>In: </xsl:text></xsl:when>
			<xsl:when test="$_LCID='2052'"><xsl:text>见：</xsl:text></xsl:when>
			<xsl:otherwise><xsl:text>In: </xsl:text></xsl:otherwise>
		</xsl:choose>
	</xsl:template>

	<xsl:template name="templ_prop_OpenParenthesis" >
		<xsl:param name="LCID" />
		<xsl:variable name="_LCID">
			<xsl:call-template name="localLCID">
				<xsl:with-param name="LCID" select="$LCID"/>
		...
		<xsl:choose>
			<xsl:when test="$_LCID='2052'"><xsl:text>（</xsl:text></xsl:when>
			<xsl:otherwise><xsl:text>(</xsl:text></xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<xsl:template name="templ_prop_CloseParenthesis" >
		<xsl:param name="LCID" />
		<xsl:variable name="_LCID">
			<xsl:call-template name="localLCID">
				<xsl:with-param name="LCID" select="$LCID"/>
				...
				<xsl:for-each select="b:Author/b:Inventor/b:NameList/b:Person">
					...
					<xsl:choose>
						<xsl:when test="$cInventorLastName=1">
							...
							<xsl:if test="position() &lt; $cInventor">
								<xsl:call-template name ="templ_prop_ListSeparator"/>
							</xsl:if>
						</xsl:when>
					</xsl:choose>
	...
	<xsl:template name="BibDisplayTitle">
		...
		<xsl:if test ="$cTitle!=0">
			...
			<!-- <xsl:call-template name ="templ_prop_Dot"/> -->
			<xsl:if test="substring(b:Title, string-length(b:Title)) != '.'">
				<xsl:text>.</xsl:text>
			</xsl:if>
			<xsl:call-template name ="templ_prop_Space"/>
		</xsl:if>
	...
	<xsl:template name="BibDisplayTitleInt">
		...
		<xsl:if test ="$cTitle!=0">
			...
			<!-- <xsl:call-template name ="templ_prop_Dot"/> -->
			<xsl:if test="substring(b:Title, string-length(b:Title) - 1) = '.'">
				<xsl:call-template name ="templ_prop_Dot"/>
			</xsl:if>
			<xsl:call-template name ="templ_prop_Space"/>
		</xsl:if>
	...
	<xsl:template name ="BibDisplayJournalNameAP">
		...
		<xsl:if test ="$cJournalName!=0">
			...
			<xsl:if test="$cIssue!=0">
				<!-- <xsl:call-template name ="templ_prop_OpenParenthesis"/> -->
				<xsl:text>(</xsl:text>
				<xsl:value-of select="b:Issue"/>
				<!-- <xsl:call-template name ="templ_prop_CloseParenthesis"/> -->
				<xsl:text>)</xsl:text>
			</xsl:if>
			<xsl:text>: </xsl:text>
	...
	<xsl:template name="BibDisplayConferenceName">
		...
		<xsl:variable name="str_InNameCap">
			<xsl:call-template name="templ_str_InCap"/>
		</xsl:variable>
		<xsl:variable name="prop_EnumSeaparator">
			<xsl:call-template name ="templ_prop_Space"/>
		</xsl:variable>
                ...
                $SourceType = 'ConferenceProceedings' ">
					<html xmlns="http://www.w3.org/TR/REC-html40">
						<body>
							...
							<xsl:if test="b:FirstAuthor">[</xsl:if>
							...
							<xsl:if test="b:LastAuthor">]</xsl:if>

							<!--xsl:if test="not(b:LastAuthor)">
								<xsl:call-template name="templ_prop_ListSeparator"/>
								<xsl:call-template name ="templ_prop_Space"/>
							</xsl:if-->
							<xsl:if test="not(b:LastAuthor)">, </xsl:if>
							</sup>
						</body>
					</html>
				</xsl:when>
