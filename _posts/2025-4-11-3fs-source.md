    ...
    - MetaClient::create
        ...
        - result = IMetaServiceStub::create
          if result->needTruncate
            - truncateImpl(..., targetLength = 0)

- meta/service                                                      ######## meta service impl ########
    - MetaOperator::create
        - runOp(MetaStore::tryOpen, req)                            # try open first
            - meta/store/ops
                - MetaStore::tryOpen
                - meta/store/Operation::run                         # run meta op with transaction and retry
                - OpenOp::run                                       # run open op with transaction context
                        ...
                        - ... check inode type ...
                          ...
                          if O_TRUNC { replaced = replaceExistsFile(...); if replaced { return replaced_inode } }
                            ...
                            - if !otrunc_replace_file || nlink != 1 || inode.len < otrunc_replace_file_threshold { return false }
                              ...
                              createInodeAndEntry(...)
                              createSession(...)
                          createSession(...)                        # store the write session in kv
          node = distributor_->getServer(req.path.parent: InodeId)  # randomly choose a meta server to perform the create op
            ...
            - fbs/meta/Utils/Weight::selectImpl                     # straw from all active nodes, whose NodeId hashed with InodeId is biggest wins
          if node == distributor_->nodeId()
            - runInBatch<Create>(req.path.parent: InodeId, req)
          else
            - forward_->forward(req)
          TODO how chains are selected?
```
