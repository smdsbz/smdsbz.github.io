...
- 有穷自动机（FA）
    ...
    - 不确定有穷自动机（NFA）
        ...
        与 DFA 不同的是：
        - $f$ 是 $K \times \Sigma \cup \left\{ \epsilon \right\} \rightarrow \rho(K)$ 映射，其中 $\rho(K)$ 表示 $K$ 的幂集；
        - $S \subset K$，称为开始状态集。
    ...
    - NFA 到 DFA 转换
        ...
        5. 计算接收状态集 $Z' = \left\{ q \vert q \in K', q \cap Z \neq \Phi \right\}$。

    - DFA 最小化（分割法）
        1. 状态集 $K$ 划分为两个状态子集 $\left\{ Z, K - Z \right\}$，记为 $\Pi = \left\{ Z, K - Z \right\}$；
        2. 如果 $\exists I \in \Pi$，$\exists a \in \Sigma$，$\exists J \in \Pi [ M(I, a) \not\subset J ]$，即状态子集 $I \in \Pi$ 中至少存在两个 $p$ 和 $q$，使得 $f(p, a) \in J'$ 和 $f(q, a) \in J''$，且 $J' \neq J''$，则将 $I$ 分割成 $I'$ 和 $I''$，即 $I' = \left\{ r \vert \forall r \in I [ f(r, a) \in J' ] \right\}$，$I'' = I - J'$；重置划分 $\Pi$：$\Pi \leftarrow (\Pi - \left\{ I \right\}) \cup \left\{ I', I'' \right\}$。
...
- $\text{FIRST}$ 集
    ...
    设文法 $G = (V_N, V_T, P, S)$，则 $\text{FIRST}(\alpha) = \left\{ a \vert \alpha \overset{\star}{\Rightarrow} a \beta, a \in V_T, \alpha, \beta \in V* \right\}$  
    特别地，$\alpha \overset{\star}{\Rightarrow} \epsilon$，约定 $\epsilon \in \text{FIRST}(\alpha)$。

    - 计算方法（$X \in V_N \cup V_T$）
        1. 对于所有终结符号 $X$，$\text{FIRST}(X) = \left\{ X \right\}$；
        2. 对于所有空规则 $X \rightarrow \epsilon$，$\text{FIRST}(X) = \left\{ \epsilon \right\}$；
        3. 对于所有形如 $X \rightarrow a \cdots$ 的规则，且 $a \in V_T$，$\text{FIRST}(X) \cup = \left\{ a \right\}$；
        4. 对于所有形如 $X \rightarrow Y_1 Y_2 \cdots Y_3$ 的规则，  
            若 $Y_1 \overset{\star}{\Rightarrow} \epsilon$，$Y_2 \overset{\star}{\Rightarrow} \epsilon$，$\cdots$，$Y_{i-1} \overset{\star}{\Rightarrow} \epsilon$（$i \leq n$），则 $\text{FIRST}(X) \cup = \cup_{j = 1}^{i} \text{FIRST}(Y_j) - \left\{ \epsilon \right\}$；  
            若 $Y_1 \overset{\star}{\Rightarrow} \epsilon$，$Y_2 \overset{\star}{\Rightarrow} \epsilon$，$\cdots$，$Y_n \overset{\star}{\Rightarrow} \epsilon$，则 $\text{FIRST}(X) \cup = \cup_{j = 1}^{n} \text{FIRST}(Y_j) \cup \left\{ \epsilon \right\}$；
        5. 重复步骤 4，直至 $\text{FIRST}$ 集不再扩大为止。

- $\text{FOLLOW}$ 集

    设文法 $G = (V_N, V_T, P, S)$，则 $\text{FOLLOW}(A) = \left\{ a \vert S \overset{\star}{\Rightarrow} \alpha A \beta, A \in V_N, a \in \text{FIRST}(\beta), \alpha, \beta \in V* \right\}$。  
    （或者 $\text{FOLLOW}(A) = \left\{ a \vert S \overset{\star}{\Rightarrow} \cdots A a \cdots, A \in V_N, a \in V_T \right\}$）

    - 计算方法（$X \in V_N$）
        1. 置 $\text{FOLLOW}(S) = \left\{ \# \right\}$；
        2. 对所有规则：  
            若 $A \rightarrow \alpha B \beta$，且 $B \in V_N$，则 $\text{FOLLOW}(B) \cup = \text{FIRST}(B) - \left\{ \epsilon \right\}$；  
...
- $\text{SELECT}$ 集
    ...
    \text{SELECT}(A \rightarrow \alpha) = \begin{cases}
        ...
        &(\text{FIRST}(\alpha) - \left\{ \epsilon \right\}) \cup \text{FOLLOW}(A)  &(\alpha \overset{\star}{\Rightarrow} \epsilon)
    \end{cases}
    $$
